...
function updateRangeFill(value) {
  const safeValue = Math.min(Math.max(Number(value) || LIMITS.minTarget, LIMITS.minTarget), LIMITS.maxTarget);
  const percent = ((safeValue - LIMITS.minTarget) / (LIMITS.maxTarget - LIMITS.minTarget)) * 100;
...
function renderCalculator({ persist = true } = {}) {
  ...
  const rangeTarget = Math.min(Math.max(Number(result.target) || LIMITS.minTarget, LIMITS.minTarget), LIMITS.maxTarget);
  elements.range.value = rangeTarget;
  elements.targetOutput.value = Number.isFinite(Number(result.target)) ? `${result.target} ELO` : '— ELO';
  updateRangeFill(rangeTarget);

  elements.current.closest('.field__control')?.classList.toggle('is-invalid', result.field === 'current');
  elements.target.closest('.field__control')?.classList.toggle('is-invalid', result.field === 'target');
...
function openOrderModal() {
  ...
  if (!currentOrderPayload?.valid) return showToast(currentOrderPayload?.error || 'Сначала укажите корректное ELO');
...
async function submitOrder(event) {
  ...
  renderCalculator({ persist: false });
  if (!currentOrderPayload?.valid) {
    elements.orderStatus.textContent = currentOrderPayload?.error || 'Проверьте значения ELO';
    return;
  }
